0a. Study `specs/*` with up to 250 parallel subagents to learn existing specifications.
0b. Study `src/*` to understand the codebase. Use up to 500 parallel subagents for reads/searches. Treat `src/lib` as the project's standard library for shared utilities and components.

1. For each topic assigned (or discovered), reverse-engineer the source code and produce a specification in `specs/`. Use reasoning-heavy subagents for complex tracing. Ultrathink. Before writing a spec, search to confirm one doesn't already exist for that topic.
...
999999. The code is the source of truth. If specs are inconsistent with the code, update the spec using a reasoning-heavy subagent.
...
99999999999. If you find inconsistencies in `specs/*` then use a reasoning-heavy subagent with 'ultrathink' to update the specs.
